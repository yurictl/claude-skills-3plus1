...
1. **Grader skill** (`/review`) — grades manifest changes against a severity-tagged rubric doc.
2. **PreToolUse hook** — deterministically blocks `git push` until the grade is fresh. "Always before X" belongs in a hook, not in a CLAUDE.md sentence the model may skip.
3. **Golden set + replay** — historical violations as fixtures; `make replay` feeds each to a headless grader (`claude -p`) and reports catch-rate. This is what makes the grader trustworthy rather than decorative — and it catches regressions when you edit the rubric or switch models.
...
3. Say `/review`. The grader reads the rubric, reports `[BLOCKER] pinned image tag — app/k8s/demo-violation.yaml ...`, and refuses to open the gate.
4. Ask Claude to fix the finding, then `/review` again — zero blockers, the gate marker is written, push proceeds (or fails on "no remote", which is fine for the demo — the gate already did its job before the push ran).
5. `make replay` — replays the golden set through a headless grader and prints the catch-rate (expect 3/3).
6. `make reset` — cleans up.
